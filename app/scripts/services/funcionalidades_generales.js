 ...
 */
angular.module('funcionalidadesGeneralesService', [])
        ...
        return {
            ...
            */
            obtener_doc: function (pago_mensual_id) {
                return new Promise((resolve, reject) => {
                    var documentos = {}
                    cumplidosMidRequest.get('contratos_contratista/documentos_pago_mensual/' + pago_mensual_id).then(function (response) {
                        if (response.data.Status === "200") {
                            documentos = response.data.Data;
                            angular.forEach(documentos, function (soporte) {
                                soporte.Documento.Metadatos = JSON.parse(soporte.Documento.Metadatos);
                            });
                        } else {
            ...
            */
            getDocumento: function (file_base64, nameWindow) {
                var file = new Blob([utils.base64ToArrayBuffer(file_base64)], { type: 'application/pdf' });
                var fileURL = URL.createObjectURL(file);
                $window.open(fileURL, nameWindow, 'resizable=yes,status=no,location=no,toolbar=no,menubar=no,fullscreen=yes,scrollbars=yes,dependent=no,width=700,height=900');
            }
        }
    });